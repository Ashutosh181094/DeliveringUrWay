...
public class EditRentalProductFragment extends Fragment implements ChangePhotoDialog.OnPhotoRecievedListener
{
    ImageView vendorImage;
    ImageView image;
    private String mSelectedImagePath;
    DatabaseReference productinfo;
    EditText costperhour,costperday;
    ...
    public View onCreateView(@NonNull LayoutInflater inflater, @Nullable ViewGroup container, @Nullable Bundle savedInstanceState) {
        View view=inflater.inflate(R.layout.edit_rental_product_fragment,container,false);
                ...
                for(int i = 0; i< Init.PERMISSIONS.length; i++)
                {
                    ...
                    if(((VendorData)getActivity()).checkPermissions(permission))
                    {
                        if(i==Init.PERMISSIONS.length-1)
                        {
                            ...
                            dialog.setTargetFragment(EditRentalProductFragment.this,0);
                        }
                    }
                    else
                    {
